...
func _ready() -> void:
	lb_client = starter.leaderboard_client
	if lb_client:
		lb_client.submit_finished.connect(_on_submit_finished)
	else:
		is_enabled = false
...
func _physics_process(delta: float) -> void:
	...
	# format-only gate for the prompt; other checks happen in the gdext library
...
func try_submitting_record() -> void:
	...
	var map_name: String = starter.map_names[starter.map_id]
	var score: int = int(Data.values.score)
	var godlikes: int = int(Data.values.get("godlikes", 0))
	var time_sec: int = int(Data.values.get("lasted", 0))
	...
	lb_client.version = ProjectSettings.get_setting("application/thunder_settings/version", 0)
	lb_client.game = "MINIX"
	lb_client.submit_record(score, godlikes, time_sec, map_name, username)


func _on_submit_finished(ok: bool, response_code: int, body: String) -> void:
	print("submit_finished ok=", ok, " code=", response_code, " body=", body)
	submitting = false
