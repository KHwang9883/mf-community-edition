...
@export_enum("Left:-1", "Right:1") var direction: int = -1:
	set(value):
		direction = value
		$Launcher.flip_h = direction > 0
		$Launcher/PosBullet.position.x *= -absf(direction)
...
func _on_bullet_launched() -> void:
	...
	NodeCreator.prepare_ins_2d(bullet_bill, self).create_2d().call_method(
		func(bul: Node2D) -> void:
			...
			if bul is GeneralMovementBody2D:
				...
				bul.vel_set(Vector2.RIGHT * bullet_speed * direction)
	...
	NodeCreator.prepare_2d(explosion, pos_bullet).create_2d().bind_global_transform(Vector2.RIGHT * 16 * direction)
