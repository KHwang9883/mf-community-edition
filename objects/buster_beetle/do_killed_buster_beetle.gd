...
func _ready() -> void:
	...
	var based_sprite: AnimatedSprite2D
	if is_instance_valid(enemy_attacked._center.based):
		var brick: Node2D = enemy_attacked._center.based
		if brick.has_node(^"Sprite"):
			based_sprite = brick.get_node(^"Sprite").duplicate()
			death_node.add_child(based_sprite)
			based_sprite.position = brick.position
			based_sprite.position.y *= -1
