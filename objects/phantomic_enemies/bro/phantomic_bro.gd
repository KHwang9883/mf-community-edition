	...
	match _step_attacking:
		...
		1:
			...
			NodeCreator.prepare_ins_2d(projectile, self).call_method(
				func(proj: Projectile) -> void:
					proj.belongs_to = Data.PROJECTILE_BELONGS.ENEMY
					
					var g: float = proj.GRAVITY * proj.gravity_scale
					var vy: float = (_last_player_pos.y - position.y) - 0.5 * g
					var vx: float = _last_player_pos.x - position.x
					
					proj.speed = Vector2(vx, vy)
					
			).create_2d().bind_global_transform(pos_attack.position)
