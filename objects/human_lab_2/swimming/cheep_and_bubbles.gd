...
func update_dir() -> void:
	var before: int = dir
	super()
	#print_rich("[color=red][b] DIRECTION BEFOR AND AFERT: ", before, " ", dir, "[/b][/color]")
	# Initial set of dir variable cheking if we need to flip it
	if before == 0 && dir < 0:
		_flip_bubble_gens()
	elif dir != before && before != 0:
		_flip_bubble_gens()
...
func _flip_bubble_gens() -> void:
	for generator: BubbleGenerator in find_children("*", "BubbleGenerator"):
