...
const MANAGER_RELOAD = preload("res://objects/human_lab_2/ramireys/manager_clone/sfx/manager_reload.wav")
...
func _on_timer_timeout() -> void:
	...
	sprite_node.play(&"stand")
	speed.x = 0
	turn_sprite = false
	update_dir()
	sprite_node.flip_h = dir < 0
	
	Audio.play_sound(MANAGER_RELOAD, self, false)
	bullets_left = 4
	timer.start(3.6)
	timer_shooting.start(0.4)
